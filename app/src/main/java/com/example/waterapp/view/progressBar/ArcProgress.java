package com.example.waterapp.view.progressBar;
...
import com.example.waterapp.utils.ProgressBarUtils;
...
public class ArcProgress extends View {
    ...
    private final int default_finished_color = Color.CYAN;
    ...
    public ArcProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        default_text_size = ProgressBarUtils.sp2px(getResources(), 18);
        min_size = (int) ProgressBarUtils.dp2px(getResources(), 100);
        default_text_size = ProgressBarUtils.sp2px(getResources(), 32);
        default_suffix_text_size = ProgressBarUtils.sp2px(getResources(), 16);
        default_suffix_padding = ProgressBarUtils.dp2px(getResources(), 4);
        default_suffix_text = "OZ";
        default_bottom_text_size = ProgressBarUtils.sp2px(getResources(), 10);
        default_stroke_width = ProgressBarUtils.dp2px(getResources(), 4);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //Bottom text height position
        arcBottomHeight = radius * (float) (1.6 - Math.cos(angle / 180 * Math.PI));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String totalText = String.valueOf(getMax());

        // Progress string
        String text = progressText + "/" + totalText;
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            // Height of Progress text baseline
            float textBaseline = (getHeight() - textHeight - 50 ) / 2.0f;

            // Main progress text position
            canvas.drawText(text, ((getWidth() - textPaint.measureText(text)) / 2.0f) - 20, textBaseline, textPaint);
            textPaint.setTextSize(suffixTextSize);
            float suffixHeight = textPaint.descent() + textPaint.ascent();

            // Sufix text position
            canvas.drawText(suffixText, getWidth() / 2.0f + textPaint.measureText(text) + suffixTextPadding - 30, textBaseline + textHeight - suffixHeight - 10 , textPaint);
        }
